...
# Canonical article body (matches blog_prompt.md pillar structure + content package).
BODY = """# How to Prepare for JEE at Home: A Complete Self-Study Blueprint (Without Burning Out)
...
def update_jee_blueprint_body(apps, schema_editor):
    BlogPost = apps.get_model("blog", "BlogPost")
    excerpt = (
        "Prepare for JEE at home with a weekly timetable, NCERT→practice bridge, mock analysis loop, "
        "and distraction boundaries—without burning out."
    )
    meta_description = (
        "Prepare for JEE at home with a weekly timetable, NCERT→practice bridge, mock analysis loop, "
        "and distraction boundaries—without burning out."
...
class Migration(migrations.Migration):
    dependencies = [
        ("blog", "0005_seed_jee_at_home_blueprint_post"),
    ]

    operations = [
        migrations.RunPython(update_jee_blueprint_body, noop_reverse),
    ]
